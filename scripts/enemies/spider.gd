...
var _attack = preload ("res://scenes/attacks/spider_bullet.tscn")
...
func _physics_process(_delta: float) -> void:
	...
	# handle movement
	if nav_agent.distance_to_target() > GameVariables.SPIDER_ATTACK_RADIUS:
		_can_attack = false
		var direction = to_local(nav_agent.get_next_path_position()).normalized()
		velocity = direction * GameVariables.SPIDER_SPEED
	else:
		_can_attack = true
		velocity *= 0.95
	move_and_slide()
...
func _on_timer_timeout() -> void:
	super()
	if _can_attack:
		_timer.wait_time = GameVariables.SPIDER_ATTACK_TIME
		_timer.start()
		_spawn_attack()
	else:
		_timer.wait_time = GameVariables.MOB_REFRESH_TIME
